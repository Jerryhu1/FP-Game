module Model.GameState where
    ...
    checkifMovePlayer gs p  | checkCollisionField p $ grid gs     = newP
            ...
            where newP       = checkCollisionBombs (explosions gs) $ colEnemies
                  colEnemies = checkCollisionEnemies (enemies gs) p
    ...
    checkCollisionEffect ::  GameState -> [SpeedBoost] -> GameState 
    checkCollisionEffect gs []                           = gs
    checkCollisionEffect gs ([x])  | checkCollision pl x = removeSpeedBoostFromGame (modPlayer gs (applyEffectOnPlayer x)) x
                                   | otherwise           = gs
                                   where pl = player gs
    checkCollisionEffect gs (x:xs) | checkCollision pl x = removeSpeedBoostFromGame (modPlayer gs (applyEffectOnPlayer x)) x
                                   | otherwise           = checkCollisionEffect gs xs
                                   where pl = player gs

    --removes speedboost from game, should be in a typeclass for all objects that can be removed?
    removeSpeedBoostFromGame :: GameState -> SpeedBoost -> GameState
    removeSpeedBoostFromGame gs obj = gs {speedBoosts = filter (== obj) (speedBoosts gs)} 
